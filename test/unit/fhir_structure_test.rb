...
class FHIRStructureTest < Test::Unit::TestCase
  ...
  def test_no_missing_resources_in_starburst
    ...
    assert(missing_resources.length == 0, "Missing these resources from the FHIRStructure: #{missing_resources.join(', ')}")
    assert(extra_resources.length == 0, "Unknown resources in the FHIRStructure: #{extra_resources.join(', ')}")
